...
SRC_FILE = ['VesselData', 'ShippingData']
RESULT_FILE = ['VesselResult.xlsx', 'ShippingResult.xlsx']
SHEET_NAME = ['船舶周报', '航运周报']


def main():
    data_type = input("Enter the type (0 for vessel, 1 for shipping):")
    assert (data_type in ['0', '1'])
    data_type = int(data_type)
    excel_list = os.listdir(SRC_FILE[data_type])
    ...
    for excel in tqdm(excel_list):
        if not excel.endswith('.xlsx'):
            continue
        try:
            workbook = openpyxl.load_workbook(os.path.join(SRC_FILE[data_type], excel))
        ...
        while sheet.cell(row=SRC_TITLE_ROW, column=i).value is not None:
            ...
            # Copy the value from B4 to B5, and its format and precision
